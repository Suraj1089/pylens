...
import { PackageResult, DetectedFile, DepKind } from "./types";
...
function kindLabel(kind: DepKind): string {
  if (kind === "dev") { return `<span class="kind kind--dev">dev</span>`; }
  if (kind === "optional") { return `<span class="kind kind--opt">optional</span>`; }
  if (kind === "transitive") { return `<span class="kind kind--trans">transitive</span>`; }
  return "";
}

function brandLogoSvg(size: number, className = ""): string {
...
export function buildLoadingHtml(total: number, format: string): string {
...
export function buildWebviewHtml(packages: PackageResult[], file: DetectedFile): string {
...
export function buildSidebarHtml(stats: SidebarStats | null, isScanning: boolean): string {
  const baseCss = `
  :root {
    --pl-accent: var(--vscode-button-background, #0078d4);
    --pl-accent-strong: var(--vscode-button-hoverBackground, #106ebe);
